...
// Animation section
document.addEventListener("DOMContentLoaded", function () {
  // ===== Force scroll to top / first section on load =====

  // If first section is at the top:
  // window.scrollTo(0, 0);

  // If you want to scroll specifically to #about:
  const firstSection = document.querySelector("#about");
  if (firstSection) {
    const top = firstSection.offsetTop;
    // Use a timeout so it overrides browser "scroll restore"
    setTimeout(() => {
  ...
  // ===== Scroll reveal: select all sections to animate =====

  const sections = document.querySelectorAll(
    "#about, #features, #mobile, #conversation-intelligence, #followups, #testdrive-flow, #team-performance, #benefits, #testimonials, #how-different"
  );
  // Add/remove more IDs here as needed

  // ===== Assign base + direction classes =====
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        // keep the first section always visible
        if (entry.target === sections[0]) return;
